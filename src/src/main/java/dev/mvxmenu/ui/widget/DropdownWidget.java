...
public class DropdownWidget implements MvxmenuWidget {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (textRenderer != null) {
            context.drawText(textRenderer, value, bounds.x + 4, bounds.y + (bounds.height - 8) / 2, MvxmenuTheme.TX_0, false);
        }
        if (focused) {
...
textRenderer = MinecraftClient.getInstance().textRenderer;
            if (textRenderer != null) {
                context.drawText(textRenderer, options[optionIndex], listX + 4, oy + (OPTION_HEIGHT - 8) / 2,
